...
from xdsl.ir.core import Attribute, EnumAttribute, Operation, SSAValue
...
class ReduceIndexOp(IRDLOperation):
    ...
    def verify_(self) -> None:
        ...
        if not _is_f32(in_ty.element_type):
            raise VerifyException(
                "tpu.reduce_index: Not Implemented: Only f32 input is supported"
                "for arg_max and arg_min"
            )
    ...
    def verify_(self) -> None:
        ...
        if self.mask is not None:
            ...
            if mask_ty.get_shape()[0] != expected:
                raise VerifyException(
                    f"tpu.scan: Mask and input mismatch. Expected mask of "
                    f"length: {expected}, but got {mask_ty.get_shape()[0]}."
                )
