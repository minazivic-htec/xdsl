...
from xdsl.irdl.constraints import EqAttrConstraint
...
class IotaOp(IRDLOperation):
    ...
    # assembly_format = "attr-dict `:` type($output)" -> will use generic syntax
    def __init__(
...
class MaskCastOp(IRDLOperation):
    ...
    def verify_(self) -> None:
        ...
        if in_shape[:2] != out_shape[:2]:
            raise VerifyException(
                f"tpu.mask_cast: leading two dimensions must match"
...
class ScanCountOp(IRDLOperation):
    ...
    assembly_format = "`mask` `(` $in_mask `:` type($in_mask) `)` `value` `(` $values `:` type($values) `)` attr-dict `:` type(results)"
...
class GatherOp(IRDLOperation):
    ...
    def __init__(
        ...
        dimension: int | IntegerAttr[IntegerType],  # ili [I32]
        result_type: Attribute,
    ):
        ...
        for operand in self.sources:
            ...
            for dim in range(len(shape)):
                if dim != dimension and shape[dim] != first_shape[dim]:
                    raise VerifyException(
                        "tpu.concatenate: Not implemented: Expected all operands to have the same shape outside of the concat dim."
                    )
